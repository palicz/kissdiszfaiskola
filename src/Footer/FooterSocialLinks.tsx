...
const labelHu: Record<SocialPlatform, string> = {
  facebook: 'Facebook',
  instagram: 'Instagram',
  youtube: 'YouTube',
  linkedin: 'LinkedIn',
...
const iconByPlatform: Record<SocialPlatform, React.FC<{ className?: string }>> = {
...
type Props = {
  links: FooterSocialLinkRow[] | null | undefined
}
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10">
      {items.map(({ platform, url }, i) => {
        ...
        const label = labelHu[platform]
        return (
          <a
            aria-label={label}
            className="text-stone-400 transition-colors hover:text-b-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-b-primary"
            href={url.trim()}
            key={`${platform}-${i}`}
