...
After integration tests, CI clears `payload_migrations` rows with `batch = -1` (dev schema push), runs `payload migrate` with `NODE_ENV=production`, then `next build` so static generation can connect without interactive migration prompts.
